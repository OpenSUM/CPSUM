...
def calc_rouge_from_python_implementation(hypothesis, references):
    ...
    scores = {
        "rouge1_F1": round(rouge_results["rouge-1"]["f"], 4) * 100.0,
        "rouge2_F1": round(rouge_results["rouge-2"]["f"], 4) * 100.0,
        "rougel_F1": round(rouge_results["rouge-l"]["f"], 4) * 100.0,
        "rouge1_R": round(rouge_results["rouge-1"]["r"], 4) * 100.0,
        "rouge2_R": round(rouge_results["rouge-2"]["r"], 4) * 100.0,
        "rougel_R": round(rouge_results["rouge-l"]["r"], 4) * 100.0,
        "rouge1_P": round(rouge_results["rouge-1"]["p"], 4) * 100.0,
        "rouge2_P": round(rouge_results["rouge-2"]["p"], 4) * 100.0,
        "rougel_P": round(rouge_results["rouge-l"]["p"], 4) * 100.0
    }
